...
public:
    ...
    static uint8_t Limit(const uint8_t value, const ValueType type);

    static void SplitStatusChannel(const uint8_t status_channel,
                                   Status& status,
                                   uint8_t& channel);
    static uint8_t CombineStatusChannel(const Status status, const uint8_t channel);

    static void SplitPitchBend(const uint16_t pitch_bend,
                               uint8_t& fine,
                               uint8_t& coarse);
    static uint16_t CombinePitchBend(const uint8_t fine, const uint8_t coarse);
};
